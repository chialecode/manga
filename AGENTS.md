...
5. 默认按 [A 规划 → B 执行 → A 审查](docs/dev-rules/development-workflow.md#agent-handoff)协作，复用同一计划、提交与证据；交接采用[最小信息模板](docs/templates/agent-handoff.md)。B 交付前完成自检，A 直接处理局部修复，较大缺口一次性反馈，用户只处理集中列明的必要决定。

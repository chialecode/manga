...
| push | 阶段完成、A 全部审查完成且用户明确表示执行后 | 默认本轮整体推送 1 次；远程审查有新修复时再集中推送，无每日配额 | 待推送版本验证通过、公开内容已核对、目标分支及有效授权明确 |
| 创建/更新 PR | 阶段完成且用户明确表示执行相应 PR 操作后 | 一个里程碑/明确子阶段一个 PR；审查修复更新原 PR，未完成用 draft 表达 | 说明具体行为、验证与剩余限制 |
...
提交到任务分支与 M0/M1 验收分开：有明确范围且通过相关检查的原型或修复可以在本地提交供审查，即使整个里程碑未退出。2026-09-20 用户收紧远端时机（A-24）：push、创建 PR、合并只在阶段完成且用户明确表示执行相应动作后进行，A 全部审查完成后可推荐。不能因此直接合并未完成的 PR、标记发行或覆盖已知限制。依赖真实设备的记录可保持 not-run，是否阻塞本批交付按对应阶段门槛判断。
...
使用 [PR 模板](../../.github/PULL_REQUEST_TEMPLATE.md)，不把每个 commit 单独开 PR。阶段完成且获明确 PR 授权后创建 PR，之后按授权集中更新同一 PR；只按新提交或实际状态变化查一次必要结果，不循环 `push → 等 CI → 小修 → push`，不通过反复推送调试可在本地复现的问题。

...
2026-09-20 用户明确：**大语言模型、向量模型、ASR 模型是独立用途，分别选择对应类型的模型。** 设置分别提供 LLM、Embedding、ASR 配置和能力测试；每项独立绑定 ModelProfile、模型 ID、连接/协议、凭据引用及参数。底层可显式选择同一服务商，但不得自动继承另一个用途的模型或凭据。AI runtime 的自研/PI 切换不改变这些用途绑定。

| 用途 | 输入与输出 | 路由约束 |
| --- | --- | --- |
| LLM | 文本/已授权材料 → 回复、整理、工具调用 | 不接收原始音频；对话成功只证明文本调用，不证明嵌入或转录 |
| Embedding | 授权文本片段 → 数值向量 | 独立向量模型；记录维度、模型/索引版本；缺配置时语义检索不可用，已有本地全文检索继续 |
| ASR | 经用户授权的音频 → 转录文本 | 只发送到独立 ASR 配置；缺配置时本地拒绝、不上传、不回退 LLM；后续整理只向 LLM 传授权转录文本 |

连接/模型目录按用途筛选；目录响应只是候选，不自动选择或变更当前模型。用途、配置版本和请求序号共同约束晚到探测结果；修改连接/模型后清除旧能力验证。发送前检查任务用途和模型用途，错配必须在网络前拒绝；不以试着上传音频的方法探测文本模型是否支持 ASR。未提供的 ASR/Embedding 保持未配置及 not-run，不能写成服务失败。M1a 先落实用途隔离、LLM/ASR 路由与设置；完整语义索引仍按 SEARCH-02 所属阶段交付。

### 9.2 供应商适配

采用 BYOK；按 [ADR-0007](../decisions/0007-technology-stack.md) 首版实际接入自研适配器与 PI（`@earendil-works/pi-ai`）两种 AI runtime，在设置中切换。runtime 与连接/协议/模型分别配置，启动任务时冻结所选 runtime；切换只影响后续任务，已有会话、授权、材料版本和工具证据保留，旧配置兼容自研实现。对应实时 ASR 传输使用 ws。它们留在服务侧适配器，不能接管工具授权、范围与修订保护，也不将供应商类型写入公共契约。具体模型/ASR 服务、凭据由用户按用途配置，材料范围受宿主授权约束。
...
连接配置包含 base URI、credentialRef、协议模式、模型 ID、超时与支持的参数；LLM、embedding、speech.user/speech.media 分别绑定对应类型的模型和连接。base URI 作为 API 根归一化，保留兼容服务路径前缀，不重复添加 /v1；连接测试显示最终基址，拒绝将凭据嵌入 URL。连接测试按能力分别执行，使用合成短文本/音频，不因可列模型就推定可调用工具或转录。兼容服务缺某接口时明确报缺失能力，不静默切换远端。
...
- 长录音分别记录总时长、保留人声/实际上传时长和重试量；筛除无人声旨在减少上传与处理量，实际费用仍取决于服务计价，不能把时长缩短直接写成已节省的费用。

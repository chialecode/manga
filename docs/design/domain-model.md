...
EPUB/MOBI 的重排正文进入规范化文本表示，固定版式和 PDF 页面保留稳定页面身份与页到文本/图像映射。可选文本层使用 text 定位并绑定表示版本；扫描页/漫画区域使用 image 定位，不伪造字符偏移。原文件指纹与位置映射独立于渲染缓存，解析器或页面顺序变化后可校验恢复。视频清单保留音轨/字幕轨 ID，VFR 按时间戳定位，不按固定帧率推算时间。
...
CaptureSession 保存采集时钟域、录音起点、设备选择、采样信息、采集时的音频保留选择和实际存储状态；音频附件可为长期保留、处理暂存或已清理。会话、转录与位置身份不能依赖音频文件仍然存在。CapturePositionEvent 记录：
...
2. 每个可核实的语音区间按原始采集起止偏移建立引用。缺分段时间时只能给已知采集片段的粗粒度来源，明确标为待校准/可调整；不能把 ASR 返回时间或均分文本当作精确时间戳。
3. 播放期间记录定期样本和暂停、跳转、倍速等事件，按分段时间映射来源区间。
4. 同一个连续播放区间可用来源时间起点与倍速插值；遇到跳转或切换资源必须切段。
5. ASR 段落匹配录音偏移，生成一个或多个来源锚点，不把不连续片段合成虚假的连续区间。
6. 小说取片段当时的资源修订与语义位置/选区，不用排版页码代替稳定定位；漫画保存页面身份与对应清单修订，展示页码只是视图。翻页与资源切换同样产生事件，一个语音段跨事件时拆段或保留多个锚点。

### 9.3 人声筛选与分段映射（后续实现）

一次录音可持续几十分钟。先在本地识别人声活动区间，保留必要边界余量以避免截断语音，再按 ASR 的格式、时长和大小限制准备上传块；无人声录音不上传。音乐、静音与噪声的筛选效果必须实测，不将背景对白识别等同于用户口述识别，也不预设具体筛选库或模型。

映射链为：**原始采集区间 → 人声片段/上传块 → 转录段 → 来源锚点**。人声片段有稳定身份及原始起止偏移；上传块保存自身偏移到原始区间的映射，拼接后的间隙、边界余量和重叠部分可追溯。返回段时间先转换到原始采集轴，再结合 CapturePositionEvent 解析小说位置、漫画页或动画播放时间。筛掉十分钟静音不能把其后的原始采集位置前移十分钟；去重也不能重复创建笔记或丢失锚点。

提供者仅返回纯文本时，可采用独立且已有时间映射的语音块建立块级转录，或经验证的对齐能力补时间；不能伪造句/词级时间戳。一个块跨多个来源边界时保留多锚点并标明粒度，无法精确对应的部分允许手动校准，不计作精确分段验收通过。取消、重试和晚到结果固定原会话、片段和映射修订，不绑定返回时的阅读焦点。

### 9.4 原文、整理与修订

录音 → Transcript 原文 → DerivedArtifact 整理候选 → 用户可编辑的笔记。各阶段保存输入和输出关系。

保留音频时，原始片段成为受保护附件，可按原始区间回放。选择不保留时，处理所需音频仅暂存；成功路径必须先确认转录文本、原始区间、分段身份和来源映射持久化，再清理暂存与上传派生音频。全程无人声仍保存会话结果与位置记录，再按策略清理。清理失败显示待清理并支持重试，不能提前显示已删除。

失败/取消不删除已确认长期保存的片段。临时音频按明确的失败/取消清理策略显示可恢复、待清理或已清理；具体时机见 Q-05，尚未定稿前原型不自动删除。重新识别或整理形成新尝试/新修订，只有音频可用时才承诺无需重新录制的 ASR 重试。音频清理后，笔记与转写仍保留原始时间间隔、来源和修订，并明确“未保留/已清理”，不能显示可播放的假入口。设置变更只影响后续采集，不追溯删除既有附件；本地清理不承诺远端服务同步删除。

本节新增筛选、保留与回顾契约属于 A-30：M1b 基础与小说映射、M2 漫画/动画扩展，当前原型尚未实现，验收见 AT-57—59。
...
正式存储适配采用 [ADR-0007](../decisions/0007-technology-stack.md) 选定的 better-sqlite3 + Drizzle；当前 M0 仍为 node:sqlite。驱动/ORM 替换须保持本文对象身份、修订、事务事件、迁移与资料包协议，不能因更换库重建用户数据或直接采用其他产品的数据表。
...
这些是逻辑分区，不要求固定在同一物理目录。用户可配置资源（小说/漫画/视频）、工程（笔记/剪辑等）、数据库与配置、附件/录音、下载/暂存、缓存/索引、日志/崩溃报告、备份、导出与运行时下载资产的根位置。平台解析系统 Documents 后，默认使用其下 MANGA 的 release 或 development 独立子树；测试使用合成数据及独立临时根。安装目录只放程序文件。

位置配置记录配置修订、发布通道、逻辑分区、目录句柄和实际解析位置；默认根与分区覆盖均在设置可见、可改。系统 Documents 可能重定向或不可用，不能猜测用户目录或静默改写到另一个位置；不可写时提供选目录/重试入口。凭据用系统保护机制加密，应用拥有的密文载荷随所配置 Profile 存放，开发/正式使用独立命名空间；不承诺迁移平台密钥本身。

改变托管位置是可恢复迁移：暂停相关写入、生成计划和检查点、验证复制后的指纹/一致性，再切换位置配置；失败保留旧数据和回退入口，不自动删除旧文件。仅索引的外部原件不会因改变默认根而移动。资源 ID、引用、任务与修订保持不变，路径变化不能创建重复资源。

启动定位机制须在 Electron 初始化前确定所有写入目录，涵盖 userData/sessionData、缓存、日志和崩溃转储。可用启动参数或环境配置定位 Profile；如需持久化启动指针，其路径也必须可配置并在设置展示。机制仍待技术验证，不能暗中固定一个不可配置的业务副本。附件清理继续按引用、任务状态与用户保留策略执行。

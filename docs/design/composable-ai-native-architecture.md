...
UI 框架、编辑基础和 SQLite 驱动已按 [ADR-0007](../decisions/0007-technology-stack.md)选定，正式接入仍需验证。运行时候选的统一契约结果、维护成本与目标平台验证、Bangumi 字段策略与 BitTorrent 引擎，以及第三方执行隔离方案仍需定稿。上述选择不改变本文规定的本地身份、受控调用、生命周期和数据所有权边界；React 组件卸载不等同业务模块停用。

...
首批正式下载采用 BitTorrent，具体引擎在 M4 接入时决定。来源可以提供种子或 magnet，实际适配范围逐项验证；Bangumi 只提供元数据，不由关联推导下载源。M0 的受控 HTTP 实验继续证明获取/入库边界，不证明 BT 支持。

BT 计划记录内容身份（info hash）、文件选择、目标目录、分片验证与引擎/恢复版本；多文件路径同样检查越界。断点数据与引擎版本绑定，换引擎不盲用旧状态。任务展示元数据获取、下载、校验、完成、上传/做种和停止状态；用户能控制上传，做种默认和带宽策略后续明确。只下载有权获取的资源，受控种子/本地 peer 用于自动验证。网盘与其他传输后续独立适配，不内置到资源身份。
...
M2 首批元数据适配 Bangumi，保留原站 ID、字段语言、来源与抓取时间；认证、限流、缺字段和人工覆盖按同一协议验证，默认字段策略在接入时定稿。
